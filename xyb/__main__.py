...
def main() -> None:
    ...
    if cmd == "install":
        ...
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "scan":
            # graphify medical scan <file> [--config xyb.toml]
            from .extract_medical import extract_from_text, extract_from_image, extract_xlsx_structured, create_llm_client_from_config
            import argparse as _ap
            import sys as _sys
            from pathlib import Path as _Path

            p = _ap.ArgumentParser(prog="graphify medical scan")
            p.add_argument("file", help="File to scan (txt, image, xlsx)")
            p.add_argument("--config", default="xyb.toml", help="Config file with LLM settings")
            p.add_argument("--output", default="graphify-out/medical_extraction.json", help="Output JSON path")
            opts = p.parse_args(_sys.argv[3:])

            src = _Path(opts.file)
            if not src.exists():
                print(f"error: file not found: {src}", file=_sys.stderr)
                _sys.exit(1)
            ...
            try:
                ...
                print("error: toml package required. Install with: pip install toml", file=_sys.stderr)
                _sys.exit(1)
            cfg = toml.load(opts.config) if _Path(opts.config).exists() else {}
            medical_cfg = cfg.get("medical", {})
            llm_cfg = medical_cfg.get("llm", {})
            client = create_llm_client_from_config(llm_cfg)
            if client is None:
                print("error: LLM config missing in xyb.toml [medical.llm]", file=_sys.stderr)
                print("Example:", file=_sys.stderr)
                print("  [medical.llm]", file=_sys.stderr)
                print("  api_base = "https://api.openai.com/v1"", file=_sys.stderr)
                print("  api_key = "sk-..."", file=_sys.stderr)
                print("  model = "gpt-4o"", file=_sys.stderr)
                _sys.exit(1)

            # Extract based on file type
            if src.suffix.lower() in (".jpg", ".jpeg", ".png", ".gif", ".webp", ".bmp", ".tiff", ".tif"):
                result = extract_from_image(src, client)
            elif src.suffix.lower() in (".xlsx", ".xls"):
                try:
                    import openpyxl
                except ImportError:
                    print("error: openpyxl package required. Install with: pip install openpyxl", file=_sys.stderr)
                    _sys.exit(1)
                ...
                result = extract_xlsx_structured(sheets, str(src))
            else:
                # Assume text file
                try:
                ...
                result = extract_from_text(text, str(src), client)

            # Save output
            out = _Path(opts.output)
            out.parent.mkdir(parents=True, exist_ok=True)
            import json as _json
            out.write_text(_json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8")
            print(f"Extraction saved to {out}")
            nodes = result.get('nodes', [])
            edges = result.get('edges', [])
            timeline = result.get('timeline_events', [])
            print(f"Extracted: {len(nodes)} nodes, {len(edges)} edges, {len(timeline)} timeline events")

        elif subcmd == "init":
            # graphify medical init [--dir PATH]
            from .init_cmd import generate_init
            import argparse as _ap
            import sys as _sys
            from pathlib import Path as _Path

            p = _ap.ArgumentParser(prog="graphify medical init")
            p.add_argument("--dir", default=".", help="Target directory")
            opts = p.parse_args(_sys.argv[3:])

            root = _Path(opts.dir)
            result = generate_init(root)
            print(f"Created {result['dirs_created']} directories and {result['files_created']} files under {result['root']}")

        elif subcmd == "report":
            # graphify medical report [--graph PATH]
            import argparse as _ap
            import sys as _sys
            from pathlib import Path as _Path

            p = _ap.ArgumentParser(prog="graphify medical report")
            p.add_argument("--graph", default="graphify-out/graph.json", help="Graph JSON path")
            p.add_argument("--format", default="md", choices=["md", "txt"], help="Output format")
            opts = p.parse_args(_sys.argv[3:])

            gp = _Path(opts.graph)
            if not gp.exists():
                print(f"error: graph not found: {gp}", file=_sys.stderr)
                _sys.exit(1)
            ...
            try:
                ...
                G = _jg.node_link_graph(raw)

            # Try to load medical report function
            try:
                from .report import render_medical_report
                report = render_medical_report(G)
                if opts.format == "txt":
...
', report)
                ...
                print("error: render_medical_report not implemented yet.", file=_sys.stderr)
                print("This will be available in a future update.", file=_sys.stderr)
                _sys.exit(1)

        elif subcmd == "serve":
            # graphify medical serve [--graph PATH]
            import argparse as _ap
            import sys as _sys
            from pathlib import Path as _Path

            p = _ap.ArgumentParser(prog="graphify medical serve")
            p.add_argument("--graph", default="graphify-out/graph.json", help="Graph JSON path")
            opts = p.parse_args(_sys.argv[3:])
            ...
            if not gp.exists():
                print(f"error: graph not found: {gp}", file=_sys.stderr)
                print("Run 'graphify update' first to build the graph.", file=_sys.stderr)
                _sys.exit(1)

            from .serve import serve as _serve
            _serve(str(gp))
        else:
            print("Usage: graphify medical [scan|init|report|serve]", file=_sys.stderr)
            print("  scan   <file>           Extract medical entities from a file", file=_sys.stderr)
            print("  init   [--dir PATH]     Initialize medical record directory structure", file=_sys.stderr)
            print("  report [--graph PATH]   Generate medical condition report", file=_sys.stderr)
            print("  serve  [--graph PATH]   Start MCP server with medical query tools", file=_sys.stderr)
            _sys.exit(1)
