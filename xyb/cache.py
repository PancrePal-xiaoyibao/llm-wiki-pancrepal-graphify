...
def cache_dir(root: Path = Path(".")) -> Path:
    """Returns xiaoyibao-out/cache/ - creates it if needed."""
    d = Path(root) / "xiaoyibao-out" / "cache"
...
def load_cached(path: Path, root: Path = Path(".")) -> dict | None:
    ...
    Cache value: stored as xiaoyibao-out/cache/{hash}.json
...
def save_cached(path: Path, result: dict, root: Path = Path(".")) -> None:
    ...
    Stores as xiaoyibao-out/cache/{hash}.json where hash = SHA256 of current file contents.
...
def clear_cache(root: Path = Path(".")) -> None:
    """Delete all xiaoyibao-out/cache/*.json files."""
    d = cache_dir(root)
