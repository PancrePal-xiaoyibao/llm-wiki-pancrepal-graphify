...
```powershell
# Detect Python and install xiaoyibao if needed
...
Set it as `$env:XIAOYIBAO_WHISPER_PROMPT` before running the transcription command.
...
**Whisper model:** Default is `base`. If the user passed `--whisper-model <name>`, set `$env:XIAOYIBAO_WHISPER_MODEL = "<name>"` before running the command above.
...
```powershell
New-Item -ItemType Directory -Force -Path xiaoyibao-out | Out-Null
python -c "
import sys, json
...
```powershell
python -m xiaoyibao.serve xiaoyibao-out/graph.json
```
...
```powershell
python -m xiaoyibao save-result --question "QUESTION" --answer "ANSWER" --type query --nodes NODE1 NODE2
```
...
```powershell
python -m xiaoyibao save-result --question "Path from NODE_A to NODE_B" --answer "ANSWER" --type path_query --nodes NODE_A NODE_B
```
...
```powershell
python -m xiaoyibao save-result --question "Explain NODE_NAME" --answer "ANSWER" --type explain --nodes NODE_NAME
```
...
```powershell
python -m xiaoyibao.watch INPUT_PATH --debounce 3
```
...
If vertical scrolling breaks in PowerShell after running xiaoyibao, this is caused by ANSI escape sequences from the `graspologic` library. Xiaoyibao v0.3.10+ suppresses this output, but if you still see the issue:

1. **Upgrade xiaoyibao**: `pip install --upgrade xyb`
...
4. **Skip graspologic**: uninstall it (`pip uninstall graspologic`) and xiaoyibao will fall back to NetworkX's built-in Louvain algorithm, which produces no ANSI output
