...
**In every subsequent bash block, replace `python3` with `$(cat .xiaoyibao_python)` to use the correct interpreter.**

### Step 2 - Detect files

```bash
$(cat .xiaoyibao_python) -c "
import json
...
Accumulate nodes/edges/hyperedges across all results and write to `.xiaoyibao_semantic_new.json`.
...
"
```
Clean up temp files: `rm -f .xiaoyibao_cached.json .xiaoyibao_uncached.txt .xiaoyibao_semantic_new.json`
...
n = to_obsidian(G, communities, 'xiaoyibao-out/obsidian', community_labels=labels or None, cohesion=cohesion)
print(f'Obsidian vault: {n} notes in xiaoyibao-out/obsidian/')

to_canvas(G, communities, 'xiaoyibao-out/obsidian/graph.canvas', community_labels=labels or None)
print('Canvas: xiaoyibao-out/obsidian/graph.canvas - open in Obsidian for structured community layout')
print()
print('Open xiaoyibao-out/obsidian/ as a vault in Obsidian.')
...
"
rm -f .xiaoyibao_detect.json .xiaoyibao_extract.json .xiaoyibao_ast.json .xiaoyibao_semantic.json .xiaoyibao_analysis.json .xiaoyibao_labels.json
...
If `code_only` is True: print `[xiaoyibao update] Code-only changes detected - skipping semantic extraction (no LLM needed)`, run only Step 3A (AST) on the changed files, skip Step 3B entirely, then go straight to merge and Steps 4–8.
...
Before the merge step, save the old graph: `cp xiaoyibao-out/graph.json .xiaoyibao_old.json`
Clean up after: `rm -f .xiaoyibao_old.json`
...
```bash
$(cat .xiaoyibao_python) -m xiaoyibao save-result --question "QUESTION" --answer "ANSWER" --type query --nodes NODE1 NODE2
```
...
```bash
$(cat .xiaoyibao_python) -m xiaoyibao save-result --question "Path from NODE_A to NODE_B" --answer "ANSWER" --type path_query --nodes NODE_A NODE_B
```
...
```bash
$(cat .xiaoyibao_python) -m xiaoyibao save-result --question "Explain NODE_NAME" --answer "ANSWER" --type explain --nodes NODE_NAME
```
...
Run once per project to make xiaoyibao always-on in Trae sessions:

```bash
xiaoyibao trae install       # or: xiaoyibao trae-cn install
```

This writes a `## xiaoyibao` section to the local `AGENTS.md` that instructs Trae to check the graph before answering codebase questions and rebuild it after code changes. No manual `/xiaoyibao` needed in future sessions.

> **Note:** Unlike Claude Code, Trae does NOT support PreToolUse hooks. The AGENTS.md rules are the always-on mechanism — there is no automatic graph rebuild on tool use. Run `/xiaoyibao --update` manually after code changes if the graph needs refreshing.

```bash
xiaoyibao trae uninstall     # or: xiaoyibao trae-cn uninstall   # remove the section
```
