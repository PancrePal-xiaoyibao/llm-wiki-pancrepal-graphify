...
Before the merge step, save the old graph: `cp xiaoyibao-out/graph.json .graphify_old.json`
...
```bash
$(cat .graphify_python) -m xiaoyibao save-result --question "QUESTION" --answer "ANSWER" --type query --nodes NODE1 NODE2
```
...
```bash
$(cat .graphify_python) -m xiaoyibao save-result --question "Path from NODE_A to NODE_B" --answer "ANSWER" --type path_query --nodes NODE_A NODE_B
```
...
```bash
$(cat .graphify_python) -m xiaoyibao save-result --question "Explain NODE_NAME" --answer "ANSWER" --type explain --nodes NODE_NAME
```
