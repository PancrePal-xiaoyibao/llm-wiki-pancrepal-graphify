...
from xiaoyibao.analyze import _node_community_map
...
def to_cypher(G: nx.Graph, output_path: str) -> None:
    lines = ["// Neo4j Cypher import - generated by /xiaoyibao", ""]
...
def to_obsidian(
    ...
    # Map file_type → xiaoyibao tag
    _FTYPE_TAG = {
        "code": "xiaoyibao/code",
        "document": "xiaoyibao/document",
        "paper": "xiaoyibao/paper",
        "image": "xiaoyibao/image",
    }
    ...
    for node_id, data in G.nodes(data=True):
        ...
        ftype_tag = _FTYPE_TAG.get(ftype, f"xiaoyibao/{ftype}" if ftype else "xiaoyibao/document")
        dom_conf = _dominant_confidence(node_id)
        conf_tag = f"xiaoyibao/{dom_conf}"
    ...
    for idx, cid in enumerate(sorted_cids):
        ...
        for m_idx, node_id in enumerate(sorted_members):
            ...
            canvas_nodes.append({
                "id": f"n_{node_id}",
                "type": "file",
                "file": f"xiaoyibao/obsidian/{fname}.md",
                "x": nx_x,
                "y": nx_y,
                "width": 180,
                "height": 60,
...
# Re-export xiaoyibao.export symbols for backward compatibility
from xiaoyibao.export import (
    to_json,
    to_cypher,
    to_obsidian,
    to_html,
